...
class BrandS3DataManager(BaseS3Storage):
    """
            ...
            try:
                ...
                if len(parts) > 1:
                    ...
                    if await self.file_exists(file_key):
                        self.logger.info(
                            "Удаление старого изображения бренда: %s", file_key
                        )
                        await self.delete_file(file_key)
            except Exception as e:
                self.logger.error(
                    "Ошибка удаления старого изображения бренда: %s", str(e)
                )

        try:
            result = await self.upload_file_from_content(
                file=file, file_content=file_content, file_key="brand_images"
            )
            self.logger.info("Загружено новое изображение бренда: %s", result)
            return result
        except Exception as e:
            self.logger.error("Ошибка загрузки нового изображения бренда: %s", str(e))
            raise ValueError("Не удалось загрузить файл изображения: %s", str(e))


async def get_brand_s3_manager(
    s3_client=Depends(get_s3_client),
) -> BrandS3DataManager:
    return BrandS3DataManager(s3_client)
